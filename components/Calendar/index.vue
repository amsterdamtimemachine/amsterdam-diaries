...
const emit = defineEmits<{
  (e: 'setSnippets', payload: { date: { id: string; value: Date }; scrollToSnippets?: boolean }): void;
}>();
/**
 * Computed Properties
 */
...
const dayClick = (calendarEvent: any, pointerEvent: any) => {
  ...
  if (calendarEvent.attributes?.some((attr: any) => attr.key === 'disabled')) {
    return;
  }
  if (pointerEvent.target?.parentElement) {
    // Set active state on clicked day (not optimal, but best solution for now)
    const parent = pointerEvent.target.parentElement;
    setActiveDay(parent);
    const calendarDate: Date = calendarEvent.date;
    const date = props.dates.find(d => {
      const dDate = new Date(d.value);
      return (
        dDate.getDate() === calendarDate.getDate() &&
        dDate.getMonth() === calendarDate.getMonth() &&
        dDate.getFullYear() === calendarDate.getFullYear()
      );
    })!;
    emit('setSnippets', { date, scrollToSnippets: true });
  }
};
...
onMounted(async () => {
  ...
  if (firstDate) {
    ...
    if (dateElement) {
      ...
      emit('setSnippets', { date: firstDate });
    }
  }
});
