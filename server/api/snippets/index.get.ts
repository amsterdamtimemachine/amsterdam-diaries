...
export default defineEventHandler(async event => {
  ...
  const resources = (
    ...
    LEFT JOIN Line l2 ON l1.position = l2.position + 1 AND l1.paragraphId = l2.paragraphId
    LEFT JOIN Line l3 ON l1.position = l3.position - 1 AND l1.paragraphId = l3.paragraphId
    JOIN paragraph p ON l1.paragraphId = p.id
